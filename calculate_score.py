...
def cal_accuracy_score(given_word: str, 
                       user_text: str, no_of_backspaces: int) -> float:
    """Calculates the accuracy score for typing tester"""

    wrong_letter = 0
    try:
        wrong_letter = sum([1 for i in range(len(given_word)) if given_word[i] != user_text[i]])
    except IndexError:
        wrong_letter += 1
